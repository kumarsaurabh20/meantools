...
def add_rxn_smarts_and_validate(row, c):
    ...
    #Kumar
    #April 2024
    #Keeping stereomeric rules for stereomeric reactions
    #check the number of rules in the output
    # hashed WHERE isStereo=0
    ...
    query = 'SELECT rules.smarts_id, rules.diameter, rules.direction, smarts.smarts_string FROM rules ' \
            'LEFT JOIN smarts ON rules.smarts_id = smarts.id ' \
            'WHERE (isStereo=0 OR isStereo=1) AND reaction_id=? AND substrate_id=?'
...
def main():
    ...
    '''
        This part parses the RetroRules and MetaNetX database to generate a CSV with 
        reaction_ids and their associated mass_transitions.

        Using the monoisotopic mass is recommended, with the optional flag: 
        "--monoisotopic_mass"
...
###########################
    ...
    '''
        This part rounds and analyses the mass_transitions generated in previous step 
        to make it compatible with the mass transitions that will later be identified by 
        rdkit during the virtual molecule generation process. This script also generates 
        some plots showing the transition degeneracy (answering questions such as how many 
        reactions can be attributed to a specific mass_transition number).
...
##################################
    ...
    '''
        This part validates the SMILES in RetroRules reactions with that of the metabolites 
        ...
        Using the optional flag "--use_greedy_rxn" is recommended. 
        This flag will make the validation tests ignore differences in missing or 
        extra H atoms (valence).
...
###########################
    ...
    '''
        This part maps the relationships among rules in RR. Because the molecular structures 
        in RR rules are all described in different diameters (substructures), many of these 
        substructures are identical across several rules, or are substructures of more complex 
        structures in other rules. This script identifies and output these relationship to 
        optimize the speed at which rules are tested with the metabolome data.

        For example, this script will identify that a rule involving the substructure C-N-C 
        does not need to be tested for a specific molecule if a rule involving the substructure 
        C-N was already tested and was not found in the molecule.

        "Base rules": are the reaction rules that describe substructures that cannot be 
        further decomposed in smaller substructures that are also in RetroRules. Base rules 
        represent step number one in testing the metabolome data.

        "Small rules": are the reaction rules described at their smallest diameter, as found 
        in RR.
    '''
    ...
    gizmos.print_milestone('    Loading input...', Options.verbose)
    df = pd.read_csv(val_path, index_col=None, dtype={'reaction_id': str, 'substrate_id': str, 'validated': bool})
    # ^^ reaction_id,substrate_id,diameter,direction,smarts_id,rxn_smarts,validated
