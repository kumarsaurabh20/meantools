...
def get_args():
    parser = argparse.ArgumentParser(prog='pathMassTransitions',
                                     description='Map mass features to mass transitions!',
                                     epilog='Contact - kumarsaurabh.singh@maastrichtuniversity.nl')
    
    required = parser.add_argument_group('Required arguments')
    required.add_argument('-c','--merged_clusters', default=True, action='store', required=True, help='Two-column csv with header. One mass signature per line. Format: ms_id,mz,mm')
    required.add_argument('-t','--mass_transitions', default=True, action='store', required=True, help='Output from treatMassTansitions.py')
    required.add_argument('-p','--pfam_RR_annotation',default=True, action='store', required=True, help='Nine-column csv. reaction_id, uniprot_id, Pfams, KO, rhea_id_reaction, kegg_id_reaction, rhea_confirmation, kegg_confirmation, KO_prediction')
    required.add_argument('-a','--gene_annotation',default=True, action='store', required=True, help='Two-column csv. Gene, pfam1;pfam2')
    required.add_argument('-dn', '--sqlite_db', default=True, action='store', required=True, help='Provide a name for the database!')
    required.add_argument('-tn', '--sqlite_tablename', default=True, action='store', required=True, help='Provide a name for the database table!')
    ...
    optional.add_argument('-o', '--output', required=False, default=False, action='store', help='Output results in a CSV file!')
...
def load_enzyme_input(Options, correlation_df):
    ...
    if Options.pfam_RR_annotation:
        ...
        pfam_dict = pd.read_csv(pfam_dict, index_col=None)
...
def get_transitions(masses_df, unique_transitions):
    ...
    if path_transitions_df.empty:
        gizmos.print_milestone('Could not find direct Substrate-Product relationships...', Options.verbose)
...
def main():
    ...
    #sanity checks
    gizmos.validate_file_path(Options.merged_clusters, "merged clusters")
    gizmos.validate_file_path(Options.pfam_RR_annotation, "Reaction rules")
    gizmos.validate_file_path(Options.gene_annotation, "annotation")
    gizmos.validate_sqlite_table(Options.sqlite_db, Options.sqlite_corr_tablename)
    gizmos.validate_sqlite_table(Options.sqlite_db, Options.sqlite_metabolite_tablename)
    ...
    meregd_clusters_df = pd.read_csv(Options.merged_clusters, index_col='id', delimiter=',', quotechar='"')
    ...
    correlation_df = gizmos.import_from_sql(Options.sqlite_db, Options.sqlite_corr_tablename, corr_columns, conditions = meregd_clusters_dict, structures = False, clone = False)
    ...
    masses_df = gizmos.import_from_sql(Options.sqlite_db, Options.sqlite_metabolite_tablename, mass_sig_columns, conditions = meregd_clusters_dict, structures = True)
    ...
    transitions_df = pd.read_csv(Options.mass_transitions, index_col=None)
    ...
    #transitions_df.to_csv("transitions_df.csv", index=False)
    ...
    if Options.output:
        ...
        gizmos.export_to_sql(Options.sqlite_db, solutions_df[cols], Options.sqlite_tablename, index=False)
