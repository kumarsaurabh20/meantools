...
def get_args():
    ...
    parser.add_argument('-f', '--output_csv_files', default=False, action='store', required=False, help='Output integration results in to multiple CSV files!')
...
def main(Options):
    ...
    # Kumar Mar 2023
    # script should connect to the database already geenrated via the previous script and update
    # the db with new table name
    ...
    if Options.output_csv_files:
        ...
        sql_full = "SELECT * FROM {};".format(Options.sqlite_table_name)
        create_output_files(Options.sqlite_db_name, sql_full, ['NPDB_ID', 'mm', 'source_name', 'INCHI', 'INCHI_key2', 'SMILES', 'ms_name', 'mz', 'Ion_name'], res_file_path)

        res_file_path = os.path.join(Options.output_folder, 'NPDB_entries.csv')
        sql_npdb_entries = "SELECT ms_name, structure_id, monoisotopic_mass, smile FROM {};".format(Options.sqlite_table_name)
        create_output_files(Options.sqlite_db_name, sql_npdb_entries, ['ms_name', 'structure_id', 'mm', 'SMILES'], res_file_path)

        res_file_path = os.path.join(Options.output_folder, 'observed_mm.csv')
        sql_mm = "SELECT ms_name, mz, monoisotopic_mass FROM {};".format(Options.sqlite_table_name)
        create_output_files(Options.sqlite_db_name, sql_mm, ['ms_name', 'mz', 'mm'], res_file_path)
